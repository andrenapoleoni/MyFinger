...
void basic()
{
    ...
    while ((ut_entry = getutent())!=NULL)
    {
        ...
        if(ut_entry->ut_type == USER_PROCESS)
        {
            ...
            char *start = gecos;
            sistemanumeri(&parola1,&parola2,&parola3,&parola4,gecos,start);
            ...
            if(strcmp(parola4,"")!=0)
            {
                ...
                strncpy(number2,parola4,strlen(parola4));
...
void basicprint()
{
    ...
    {
        printf("%-*s%-*s%-5s\t%s\t%s\t(%s)\t%s \n",l1+4,data[a].user,l2+4, data[a].real_name,data[a].tty,data[a].idle_time,data[a].login_time,data[a].hostname,data[a].office_number);
        a++;
    }
    a=0;
}
...
void sistemanumeri(char** parola1,char** parola2,char** parola3, char** parola4,char* gecos, char* start )
{
    ...
    while (gecos[index] != '\0') 
    {
        if (gecos[index] == ',') 
        {
            ...
            parola_num++;
            start = &gecos[index + 1];  // Punto di partenza della prossima parola
        }
